...
#define    FETCH_32(p) ({                                     \
    register uint32_t l = (uint32_t)*((const uint32_t *)(p)); \
...
static void SHA1Transform(uint32_t, uint32_t, uint32_t, uint32_t,
    uint32_t, const uint8_t *, SHA1_CTX *);
...
SHA1Final(void *digest, SHA1_CTX *context)
{
    ...
    uint32_t index = (context->bcount[1] >> 3) & 0x3f;
 ...
 */
static void
SHA1Transform(uint32_t a, uint32_t b, uint32_t c, uint32_t d,
    uint32_t e, const uint8_t block[64], SHA1_CTX *context)
{
